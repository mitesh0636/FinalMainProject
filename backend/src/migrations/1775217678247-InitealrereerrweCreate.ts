...
export class InitealrereerrweCreate1775217678247 implements MigrationInterface {
    name = 'InitealrereerrweCreate1775217678247'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "temporary_ORDER" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "TotalPayment" decimal(10,2) NOT NULL, "orderDate" datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP), "paymentMethod" varchar CHECK( "paymentMethod" IN ('credit','debit','banktransfer','cashondelivery') ) NOT NULL, "userId" integer, "TaxApplied" float, "SubTotal" float, "deliveryDate" datetime NOT NULL, "orderstatus" varchar CHECK( "orderstatus" IN ('ordered','dispatched','cancelled','delivered') ) NOT NULL, CONSTRAINT "FK_0ef741dece1ca96cbcbd08a73c9" FOREIGN KEY ("userId") REFERENCES "USERS" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION)`);
