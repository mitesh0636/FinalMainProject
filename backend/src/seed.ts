...
async function seed() {
  ...
  try {
    ...
    // ════════════════════════════════════════════
    const categoryData: { categoryname: string; typeIndex: number }[] = [
      // Electronics (0)
    ...
    // ════════════════════════════════════════════
    const subcategoryData: { subcategoryname: string; catIndex: number }[] = [
      // Mobile & Tablets (0)
    ...
    // ════════════════════════════════════════════
    // 4. PRODUCTS (~45 products)
    // ════════════════════════════════════════════
    type ProductSeed = { name: string; description: string; price: number; available: number; imagePath: string; subIndex: number };
    ...
    const userData: Partial<User>[] = [
      { name: "Admin User",       email: "admin@shopease.com",    password: adminPassword,  address: "1 Admin Lane, Mumbai", contactno: "9000000001", dateofbirth: new Date("1990-01-15"), role: Roleenum.ADMINROLE },
    ...
    for (const ud of userData) {
      const u = userRepo.create({ ...ud, isLocked: false, role: ud.role ?? Roleenum.USERROLE });
    ...
    // ════════════════════════════════════════════
    const activeCartSeeds: { cartIndex: number; productIndex: number; quantity: number }[] = [
    ...
    type OrderSeed = {
      ...
      status: OrderStatus;
      daysAgo: number;
      deliveryDaysFromOrder: number;
      items: { productIndex: number; quantity: number }[];
    };

    const orderSeeds: OrderSeed[] = [
      {
        userIndex: 1, paymentMethod: PAYMENTMETHOD.CREDITCARD, status: OrderStatus.DELIVERED, daysAgo: 30, deliveryDaysFromOrder: 5,
        items: [{ productIndex: 1, quantity: 1 }, { productIndex: 12, quantity: 1 }],
      },
      {
        userIndex: 2, paymentMethod: PAYMENTMETHOD.DEBITCARD, status: OrderStatus.DELIVERED, daysAgo: 20, deliveryDaysFromOrder: 4,
        items: [{ productIndex: 8, quantity: 1 }],
      },
      {
        userIndex: 3, paymentMethod: PAYMENTMETHOD.CASHONDELIVERY, status: OrderStatus.DELIVERED, daysAgo: 15, deliveryDaysFromOrder: 3,
        items: [{ productIndex: 16, quantity: 2 }, { productIndex: 18, quantity: 1 }],
      },
      {
        userIndex: 4, paymentMethod: PAYMENTMETHOD.BANKTRANSFER, status: OrderStatus.DISPATCHED, daysAgo: 3, deliveryDaysFromOrder: 5,
        items: [{ productIndex: 6, quantity: 1 }, { productIndex: 11, quantity: 1 }],
      },
      {
        userIndex: 5, paymentMethod: PAYMENTMETHOD.CREDITCARD, status: OrderStatus.ORDERED, daysAgo: 1, deliveryDaysFromOrder: 7,
        items: [{ productIndex: 33, quantity: 1 }],
      },
      {
        userIndex: 6, paymentMethod: PAYMENTMETHOD.DEBITCARD, status: OrderStatus.DELIVERED, daysAgo: 45, deliveryDaysFromOrder: 4,
        items: [{ productIndex: 23, quantity: 1 }, { productIndex: 24, quantity: 1 }, { productIndex: 26, quantity: 1 }],
      },
      {
        userIndex: 7, paymentMethod: PAYMENTMETHOD.CASHONDELIVERY, status: OrderStatus.DELIVERED, daysAgo: 10, deliveryDaysFromOrder: 3,
        items: [{ productIndex: 36, quantity: 1 }, { productIndex: 37, quantity: 2 }],
      },
      {
        userIndex: 8, paymentMethod: PAYMENTMETHOD.CREDITCARD, status: OrderStatus.CANCELLED, daysAgo: 7, deliveryDaysFromOrder: 6,
        items: [{ productIndex: 9, quantity: 1 }],
      },
      {
        userIndex: 9, paymentMethod: PAYMENTMETHOD.BANKTRANSFER, status: OrderStatus.DELIVERED, daysAgo: 60, deliveryDaysFromOrder: 5,
        items: [{ productIndex: 42, quantity: 3 }, { productIndex: 43, quantity: 2 }],
      },
      {
        userIndex: 10, paymentMethod: PAYMENTMETHOD.DEBITCARD, status: OrderStatus.DISPATCHED, daysAgo: 2, deliveryDaysFromOrder: 4,
        items: [{ productIndex: 3, quantity: 1 }, { productIndex: 13, quantity: 2 }],
      },
      {
        userIndex: 11, paymentMethod: PAYMENTMETHOD.CASHONDELIVERY, status: OrderStatus.DELIVERED, daysAgo: 25, deliveryDaysFromOrder: 5,
        items: [{ productIndex: 28, quantity: 1 }, { productIndex: 30, quantity: 1 }],
      },
      {
        userIndex: 12, paymentMethod: PAYMENTMETHOD.CREDITCARD, status: OrderStatus.ORDERED, daysAgo: 0, deliveryDaysFromOrder: 7,
        items: [{ productIndex: 4, quantity: 1 }],
      },
      {
        userIndex: 1, paymentMethod: PAYMENTMETHOD.DEBITCARD, status: OrderStatus.DELIVERED, daysAgo: 90, deliveryDaysFromOrder: 4,
        items: [{ productIndex: 44, quantity: 1 }, { productIndex: 45, quantity: 1 }],
      },
      {
        userIndex: 14, paymentMethod: PAYMENTMETHOD.CREDITCARD, status: OrderStatus.DISPATCHED, daysAgo: 2, deliveryDaysFromOrder: 5,
        items: [{ productIndex: 7, quantity: 1 }],
      },
      {
        userIndex: 15, paymentMethod: PAYMENTMETHOD.BANKTRANSFER, status: OrderStatus.DELIVERED, daysAgo: 12, deliveryDaysFromOrder: 3,
